...
namespace Mankalari
{
    public class BoardDrawerConsole : BoardDrawer
    {
        public override void
            DrawBoard(Board b, bool showIndex = false) //draws indexed board first (if wanted) then normal board
        {
            if (showIndex)
                DisplayBoard(b, true);
            DisplayBoard(b);
        ...
        static string
            PrintRow(int homeIndex, bool leftToRight, bool showIndex, Board b) //returns a string representing a row
        {
            string row = "|";
            ...
            {
                string cupText =
                    showIndex ? i.ToString().PadLeft(2) : $"{b.GetCup(i)}"; //show either index or cup's points
